...
def create_packet(id):
    ...
    data = 192 * 'Q' if PING_DATA == None else PING_DATA
    # Calculate the checksum on the data and the dummy header.
    my_checksum = checksum(header + data)
    # Now that we have the right checksum, we put that in. It's just easier
    # to make up a new header than to stuff it into the dummy.
    header = struct.pack('bbHHh', ICMP_ECHO_REQUEST, PACKET_CODE,
                         socket.htons(my_checksum), id, PACKET_SEQUENCE)
    return header + data
...
# MY UNPACK
def unpack_header(pack):
    type, code, checksum, p_id, sequence = struct.unpack('bbHHh', pack)
...
def my_func(dest_addr, timeout=1):
    """
    Sends one ping to the given "dest_addr" which can be an ip or hostname.
    "timeout" can be any integer or float except negatives and zero.
    ...
    # we have to sure that our packet id is not greater than that.
    packet_id = int((id(timeout) * random.random()) % 65535)
    print('WILL SEND PACKET WITH ID', packet_id)
    packet = create_packet(packet_id)
    while packet:
        # The icmp protocol does not use a port, but the function
        # below expects it, so we just give it a dummy port.
...
def fill_ping_data(data):
  global PING_DATA
  data = data[:192] if len(data) > 192 else data
  PING_DATA = data.ljust(192, '0')

if __name__ == '__main__':
    # Testing
    #verbose_ping('127.0.0.1')
    global ICMP_ECHO_REQUEST, PACKET_CODE, PACKET_SEQUENCE
    ICMP_ECHO_REQUEST = 127
    PACKET_CODE = 123
    PACKET_SEQUENCE = 123
    fill_ping_data('custom data')
    d = my_func('127.0.0.1')
    print(str(d*10e6), 'us')